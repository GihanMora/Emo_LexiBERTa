...
def run_main(subfolder,start_and_end):
    ...
    is_file_already = os.path.isfile(
        r'E:\Projects\Emotion_work_Gihan\Emo_LexiBERTa\use_case_earning_calls\unprocessed_results/line_out_fixed/results_unprocessed_lnm_transcripts_' + str(
...
if __name__ == '__main__':
    ...
    with open(r'E:\Projects\Emotion_work_Gihan\Emo_LexiBERTa\use_case_earning_calls\unprocessed_results/line_out_fixed/results_unprocessed_lnm_transcripts_' + str(
            s_e_f) + '_' + str(s_e[0]) + '_' + str(s_e[1]) + '.csv', mode='a+', newline='',
              encoding='utf-8') as employee_file:
        employee_writer = csv.writer(employee_file, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
        if (not is_file_already):
